    ...
    static void createLangFile(JavaPlugin plugin, String langName) {
        ...
        if (!lang.exists()) {
            try {
                ...
                e.printStackTrace();
                plugin.getServer().getLogger().info("[SimplePin] Could not create language file");
        ...
        try {
            ...
            plugin.getServer().getLogger().info("[SimplePin] Could not save language file");
            plugin.getServer().getLogger().info("[SimplePin] Disabling plugin");
            plugin.getServer().getPluginManager().disablePlugin(plugin);
        }
    }
